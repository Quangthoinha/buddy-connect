...
| **Preview** | Push code lên branch `dev` → Vercel auto-deploy | `<project>-git-dev.vercel.app` | `app_{slug}_dev` (cùng với local) | Bạn (qua dev_mode trong app Mushy) |
| **Production** | Merge code vào branch `main` → Vercel auto-deploy | `<project>.vercel.app` (default Vercel) | `app_{slug}` (real data) | Mọi người dùng app trong workspace |
...
Mặc định khi user mở mini-app trong app Mushy, nó load **URL prod** (`<project>.vercel.app`). Bạn (mini-app owner) muốn xem **build dev** (URL preview Vercel `<project>-git-dev.vercel.app`) để test → bật **Dev Mode**:
...
2. Connect Vercel (tài khoản của bạn) → import GitHub repo → auto-build branch `main` ra `<project>.vercel.app` + branch `dev` ra `<project>-git-dev.vercel.app`.
...
5. **2 stable URLs Vercel** (auto, không cần Cloudflare custom domain):
   ...
   |---|---|---|
   | `main` | `<project>.vercel.app` | Production — paste vào `url` ở Admin Portal |
   | `dev`  | `<project>-git-dev.vercel.app` | Preview (dev_mode) — paste vào `preview_url` ở Admin Portal |

   Vercel tự tạo cả 2 URL khi push lên 2 branch tương ứng. **Stable per branch** (không phải per-commit). Mỗi push lên branch tương ứng → cùng URL update sang deploy mới.

   **KHÔNG paste** per-deployment URL random kiểu `<project>-abc123.vercel.app` — đổi theo commit, không stable.

   ℹ️ Optional: nếu muốn URL branded `{slug}.mini.mushy-app.com`, nhờ anhdqvn manual add Cloudflare CNAME + Vercel custom domain. Default flow KHÔNG yêu cầu — `<project>.vercel.app` đủ dùng cho mọi mini-app (WebView không show URL bar, user không thấy).

6. Mở Admin Portal (https://admin.mini.mushy-app.com) → login → catalog → **+ Đăng ký app mới**:
   - Slug: `{slug}` (uniqueness check live)
   - Tên + mô tả + icon
   - **Production URL**: paste `https://<project>.vercel.app`
   - **Preview URL**: paste `https://<project>-git-dev.vercel.app`
...
- `main` = production (canonical, stable). Push lên main → Vercel deploy `<project>.vercel.app`.
- `dev` = development/preview. Push lên dev → Vercel deploy `<project>-git-dev.vercel.app`.
- Standard Git flow: code daily trên `dev` → PR/merge `dev → main` để ship prod.

Cả 2 URL `<project>.vercel.app` + `<project>-git-dev.vercel.app` được Vercel auto-cấp, không cần Cloudflare/DNS setup.
